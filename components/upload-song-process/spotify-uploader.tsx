...
export default function SpotifyUploader() {
  ...
  const removeTrackFile = (trackId: string) => {
    setTrackFiles((prev) =>
      prev.map((trackFile) => (trackFile.trackId === trackId ? { ...trackFile, file: null } : trackFile)),
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Importador de Lanzamientos de Spotify</h1>
            ...
            {[1, 2, 3].map((step) => (
              <div key={step} className="flex items-center">
                ...
                <div className="ml-3 text-sm">
                  <div className={`font-medium ${currentStep >= step ? "text-blue-600" : "text-slate-500"}`}>
                    {step === 1 && "Información"}
        ...
        <AlertDialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Estás a punto de importar {trackFiles.filter(tf => tf.file !== null).length} de {spotifyData?.totalTracks} canciones. 
                ¿Estás seguro de que deseas continuar con la importación?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
